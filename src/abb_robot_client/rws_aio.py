...
class RWS_AIO:
    ...
    async def start(self, cycle: str = "asis", tasks: List[str] = ["T_ROB1"]):
        ...
        _res = await self._do_post("rw/rapid/execution?action=start", payload)
    ...
    async def stop(self):
        ...
        _res = await self._do_post("rw/rapid/execution?action=stop", payload)

    async def resetpp(self):
        """
        Reset RAPID program pointer to main in normal tasks
        """
        _res = await self._do_post("rw/rapid/execution?action=resetpp")
    ...
    async def set_controller_state(self, ctrl_state):
        ...
        _res = await self._do_post("rw/panel/ctrlstate?action=setctrlstate", payload)
    ...
    async def set_digital_io(self, signal: str, value: Union[bool, int], network: str = "Local", unit: str = "DRV_1"):
        ...
        _res = await self._do_post(
            "rw/iosystem/signals/" + network + "/" + unit + "/" + signal + "?action=set", payload
        )
    ...
    async def get_rapid_variables(self, task: str = "T_ROB1") -> str:
        ...
        res_json = await self._do_post("rw/rapid/symbols?action=search-symbols", payload)
    ...
    async def set_rapid_variable(self, var: str, value: str, task: str = "T_ROB1"):
        ...
        if task is not None:
            ...
            var1 = var
        _res = await self._do_post("rw/rapid/symbol/data/RAPID/" + var1 + "?action=set", payload)
    ...
    async def get_jointtarget(self, mechunit: str = "ROB_1"):
        ...
        robjoint = np.array(
            [state["rax_1"], state["rax_2"], state["rax_3"], state["rax_4"], state["rax_5"], state["rax_6"]],
            dtype=np.float64,
        )
        extjoint = np.array(
    ...
    async def get_robtarget(
        ...
        extax = np.array(
            [state["eaxa"], state["eaxb"], state["eaxc"], state["eaxd"], state["eaxe"], state["eaxf"]], dtype=np.float64
        )
    ...
    async def send_ipc_message(
        self, target_queue: str, data: str, queue_name: str, cmd: int = 111, userdef: int = 1, msgtype: int = 1
    ):
        """
        ...
        payload = {
            ...
            "dipc-cmd": str(cmd),
            "dipc-userdef": str(userdef),
            "dipc-msgtype": str(msgtype),
            "dipc-data": data,
        }
        _res = await self._do_post("rw/dipc/" + target_queue + "?action=dipc-send", payload)
    ...
    async def try_create_ipc_queue(self, queue_name: str, queue_size: int = 4440, max_msg_size: int = 444):
        ...
        try:
            payload = {
                "dipc-queue-name": queue_name,
                "dipc-queue-size": str(queue_size),
                "dipc-max-msg-size": str(max_msg_size),
            }
    ...
    async def logout(self):
        try:
            _res = await self._do_get("logout")
        finally:
            try:
    ...
    async def subscribe(self, resources: List[SubscriptionResourceRequest]):
        ...
        async with self._websocket_lock:
            ...
            self._websocket = await websockets.connect(
                ws_url, extra_headers=header, subprotocols=["robapi2_subscription"]
            )
    ...
    def _init_subscription_convert_message(self):
        self._signal_re = re.compile(
            r'<a\s+href="/rw/iosystem/signals/([^"]+);state"\s+rel="self"/?>.*<span\s+class="lvalue">([^<]+)<'
        )
        self._pers_re = re.compile(
            r'<a\s+href="/rw/rapid/symbol/data/RAPID/([^"]+);value"\s+rel="self"/?>.*<span\s+class="value">([^<]+)<'
        )
        self._elog_re = re.compile(r'<a\s+href="/rw/elog/0/([^"]+)"\s+rel="self"/?>.*<span\s+class="seqnum">([^<]+)<')
        self._exec_re = re.compile(
            r'<a\s+href="/rw/rapid/execution;ctrlexecstate"\s+rel="self"/?>.*<span\s+class="ctrlexecstate">([^<]+)<'
        )
        self._opmode_re = re.compile(r'<a\s+href="/rw/panel/opmode"\s+rel="self"/?>.*<span\s+class="opmode">([^<]+)<')
        self._ctrl_re = re.compile(
            r'<a\s+href="/rw/panel/ctrlstate"\s+rel="self"/?>.*<span\s+class="ctrlstate">([^<]+)<'
        )
